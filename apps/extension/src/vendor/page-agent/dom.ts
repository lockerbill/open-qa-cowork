// @openqa-edit vendored from packages/page-controller/src/dom/index.ts (see
// ../VENDORED.md). Local changes, each marked @openqa-edit:
//  - imports flattened to sibling dom_tree.js / dom_tree.d.ts
//  - highlights off by default, behind DomConfig.debugHighlights (§4.2.4)
//  - redactNode seam in flatTreeToString (§4.2.2, upstream's "@todo 数据脱敏过滤器")
//  - index-access guards for noUncheckedIndexedAccess (no behavior change)
import domTree from './dom_tree.js'
import type {
	ElementDomNode,
	FlatDomTree,
...
export interface DomConfig {
	...
	// @openqa-edit render numbered highlight overlays (debug only; off in production runs)
...
export function getFlatTree(config: DomConfig): FlatDomTree {
	...
	const elements = domTree({
		// @openqa-edit highlights/debug off unless explicitly requested (§4.2.4)
		doHighlightElements: config.debugHighlights ?? false,
		debugMode: config.debugHighlights ?? false,
	...
	for (const nodeId in elements.map) {
		...
		if (!node) continue // @openqa-edit noUncheckedIndexedAccess guard
	...
	for (const pattern of patterns) {
		if (pattern.includes('*')) {
			...
			for (const key of Object.keys(attrs)) {
				const value = attrs[key] ?? '' // @openqa-edit noUncheckedIndexedAccess guard
				if (regex.test(key) && value.trim()) {
...
// @openqa-edit public view of TreeNode for the redactNode callback (§4.2.2)
export type TreeNodeView = TreeNode

// @openqa-edit options bag for flatTreeToString; fills upstream's
// "@todo 数据脱敏过滤器" with a per-node redaction seam. The callback runs for
// every element/text node BEFORE its text or attribute values are emitted
// (applied at tree-build time, which also covers text gathered by
// getAllTextTillNextClickableElement). Children are already built when an
// element node is passed, so a callback may also scrub descendant text.
export interface FlatTreeToStringOptions {
	redactNode?: (node: TreeNodeView) => TreeNodeView
}
 ...
 */
// @openqa-edit relied upon; pinned to base commit
// @openqa-edit optional opts.redactNode redaction seam (§4.2.2)
export function flatTreeToString(
	flatTree: FlatDomTree,
	includeAttributes: string[] = [],
	keepSemanticTags = false,
	opts: FlatTreeToStringOptions = {}
): string {
	const redactNode = opts.redactNode // @openqa-edit
	...
	const buildTreeNode = (nodeId: string): TreeNode | null => {
		...
		if (node.type === 'TEXT_NODE') {
			...
			return redactNode ? redactNode(built) : built // @openqa-edit redaction seam
		} else {
				...
				if (includeAttrs.length > 0 && node.attributes) {
					...
					if (keys.length > 1) {
						...
						for (const key of keys) {
							const value = attributesToInclude[key] ?? '' // @openqa-edit noUncheckedIndexedAccess guard
							if (value.length > 5) {
					...
					for (const attr of attrsToRemoveIfTextMatches) {
						const attrValue = attributesToInclude[attr] // @openqa-edit noUncheckedIndexedAccess guard
						if (attrValue && attrValue.toLowerCase().trim() === text.toLowerCase().trim()) {
...
export function getElementTextMap(simplifiedHTML: string) {
	...
	for (const line of lines) {
		...
		if (match && match[1] !== undefined) {
			// @openqa-edit noUncheckedIndexedAccess guard
