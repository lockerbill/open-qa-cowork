 ...
 *
 * search @openqa-edit for OpenQA's local changes (see ../VENDORED.md).
 * @openqa-edit add data-openqa-ignore to the ignore-attribute check
 */
...
export default (
   ...
   */
	...
	function buildDomTree(node, parentIframe = null, isParentHighlighted = false) {
		 ...
		 * @edit add `data-browser-use-ignore` attribute
		 */
		// @openqa-edit also skip OpenQA's own UI (stop overlay, panels) via data-openqa-ignore
		if (
			node.dataset?.browserUseIgnore === 'true' ||
			node.dataset?.pageAgentIgnore === 'true' ||
			node.dataset?.openqaIgnore === 'true'
		) {
